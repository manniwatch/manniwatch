...
export class StopPointService {
    ...
    constructor(
        ...
        this.mStopPointObservable = this.setupLocationsPoll(
            this.api.getStopPointLocations().pipe(map((stops: IStopPointLocations): IStopPointLocation[] => stops.stopPoints))
        );
    }
    ...
    public filterByObservable(filter: Observable<string>): Observable<IStopLocation> {
        return this.stopObservable.pipe(
            withLatestFrom(filter),
            map((value: [IStopLocation[], string]): IStopLocation => {
                if (value[0] && value[1]) {
                    const idx: number = value[0].findIndex((stop: IStopLocation): boolean => stop.shortName === value[1]);
                    if (idx >= 0) {
                        return value[0][idx];
                    }
                }
    ...
    public filterStop(string: string): Observable<IStopLocation> {
        return this.stopObservable.pipe(
            map((stopLocations: IStopLocation[]): IStopLocation => {
                if (stopLocations) {
                    const idx: number = stopLocations.findIndex((stop: IStopLocation): boolean => stop.shortName === string);
                    if (idx >= 0) {
    ...
    public watchStopPoint(stopPointId: string): Observable<IStopPointLocation> {
        return this.stopPointObservable.pipe(
            map((stopLocations: IStopPointLocation[]): IStopPointLocation => {
                if (stopLocations) {
                    const idx: number = stopLocations.findIndex((stop: IStopPointLocation): boolean => stop.stopPoint === stopPointId);
                    if (idx >= 0) {
                        return stopLocations[idx];
                    }
                }
