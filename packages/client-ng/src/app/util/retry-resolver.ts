...
export abstract class RetryResolver<T> implements Resolve<T>{
    ...
    public resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<T> {
        return this.createLoader(route, state)
            .pipe(retryWhen((obs: Observable<ErrorTypes>): Observable<boolean> => {
                return obs.pipe(mergeMap((err: ErrorTypes) => {
                    if (err instanceof HttpErrorResponse) {
                        if (err.status === 404) {
                            return this.navigate(['error', 'not-found'])
                                .pipe(mergeMap((navigated: boolean): Observable<never> => {
                                    return EMPTY;
                                }));
                        } else if (err.status >= 500 && err.status < 600) {
                            return this.dialog.getRetryDialog({
                                msg: 'Error'
                            }).pipe(mergeMap((it: boolean): Observable<true> => {
