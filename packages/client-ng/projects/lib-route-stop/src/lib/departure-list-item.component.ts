...
@Component({
    ...
    selector: 'mwlr-stop-departure-list-item',
    ...
    public calculateDelay(data: IDeparture): false | number {
        if (data && data.actualTime && data.plannedTime) {
            if (data.actualTime !== data.plannedTime) {
                const actual: momentNs.Moment = moment(data.actualTime, 'HH:mm');
                const planned: momentNs.Moment = moment(data.plannedTime, 'HH:mm');
