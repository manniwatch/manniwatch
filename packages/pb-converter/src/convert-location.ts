...
export const convertLocation: (cat: ILocDefinitions) => manniwatch.manniwatch.ILocation | undefined =
    (cat: ILocDefinitions): manniwatch.manniwatch.ILocation | undefined => {
        if (cat.lat && cat.lon) {
            return {
                latitude: cat.lat,
                longitude: cat.lon,
